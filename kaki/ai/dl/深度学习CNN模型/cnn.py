...
model.add(tf.keras.layers.Conv1D(512, 1, activation='relu', input_shape=x_train.shape[-2:]))
model.add(tf.keras.layers.MaxPooling1D(1))
model.add(tf.keras.layers.Flatten())
model.add(tf.keras.layers.Dense(512, activation='relu'))
model.add(tf.keras.layers.Dense(1))
model.compile(optimizer='adam', loss='mse')
model.fit(x_train, y_train, epochs=100, batch_size=100000, verbose=1, validation_data=(x_valid, y_valid),  callbacks=callbacks)
