...
model = tf.keras.models.Sequential()  # 创建序列模型
model.add(tf.keras.layers.Conv1D(256, 1, activation='relu', input_shape=x_train.shape[-2:]))
model.add(tf.keras.layers.MaxPooling1D(1))
model.add(tf.keras.layers.LSTM(256, return_sequences=True, input_shape=x_train.shape[-2:]))  # 添加第一个 LSTM 层
model.add(tf.keras.layers.LSTM(256, activation='relu'))  # 添加第二个 LSTM 层
model.add(tf.keras.layers.Dense(256, activation='relu'))  # 添加全连接层
model.add(tf.keras.layers.Dense(1, activation='sigmoid'))  # 添加输出层
model.compile(optimizer='adam', loss='mse')  # 编译模型，设置优化器和损失函数
model.fit(x_train, y_train, epochs=100, batch_size=100000, verbose=1, validation_data=(x_valid, y_valid), callbacks=callbacks)  # 使用训练集数据训练模型，并使用验证集数据进行验证
