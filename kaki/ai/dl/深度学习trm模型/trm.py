...
input_dim = int(len(x_train.T))  # 输入维度
output_dim = 1  # 输出维度
num_layers = 4  # 编码器层数
num_heads = 4  # 注意力头数
model = transformer_model(input_dim, 128, output_dim, num_layers, num_heads)
model.compile(optimizer='adam', loss='mse')
model.fit(x_train, y_train, epochs=100, batch_size=30000, verbose=1, validation_data=(x_valid, y_valid),callbacks=callbacks)
model.save('trm.keras')