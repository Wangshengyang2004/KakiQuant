...
def alpha5(Open, vwap, close):
    alpha = (rank((Open - (ts_sum(vwap, 10) / 10)))
...
def alpha14(Open, volume, returns):
    x1 = correlation(Open, volume, 10).replace(
...
def alpha15(high, volume):
    x1 = correlation(rank(high), rank(volume), 3).replace(
        [-np.inf, np.inf], 0).fillna(value=0)
...
def alpha18(close, Open):
    x = correlation(close, Open, 10).replace(
        [-np.inf, np.inf], 0).fillna(value=0)
...
def alpha20(Open, high, close, low):
    alpha = -1 * (rank(Open - delay(high, 1)) * rank(Open -
...
def alpha22(high, volume, close):
    x = correlation(high, volume, 5).replace(
        [-np.inf, np.inf], 0).fillna(value=0)
...
def alpha26(volume, high):
    x = correlation(ts_rank(volume, 5), ts_rank(high, 5), 5).replace(
        [-np.inf, np.inf], 0).fillna(value=0)
    alpha = -1 * ts_max(x, 3)
    return alpha.fillna(value=0)
...
def alpha28(volume, high, low, close):
    adv20 = sma(volume, 20)
    x = correlation(adv20, low, 5).replace(
        [-np.inf, np.inf], 0).fillna(value=0)
...
def alpha29(close, returns):
    x1 = ts_min(rank(
        rank(scale(np.log(ts_sum(rank(rank(-1 * rank(delta((close - 1), 5)))), 2))))), 5)
...
def alpha31(close, low, volume):
    ...
    x3 = np.sign(scale(correlation(adv20, low, 12).replace(
        [-np.inf, np.inf], 0).fillna(value=0)))
    alpha = x1 + x2 + x3
    return alpha.fillna(value=0)
...
def alpha32(close, vwap):
    x = correlation(vwap, delay(close, 5), 230).replace(
        [-np.inf, np.inf], 0).fillna(value=0)
...
def alpha37(Open, close):
    alpha = rank(correlation(delay(Open - close, 1),
                             close, 200)) + rank(Open - close)
    return alpha.fillna(value=0)
...
def alpha53(close, high, low):
    alpha = -1 * delta((((close - low) - (high - close)) /
...
def alpha55(high, low, close, volume):
    x = (close - ts_min(low, 12)) / \
        (ts_max(high, 12) - ts_min(low, 12)).replace(0, 0.0001)
    alpha = -1 * correlation(rank(x), rank(volume),
...
def alpha61(volume, vwap):
    ...
    alpha = rank((vwap - ts_min(vwap, 16))
...
def alpha64(high, low, Open, volume, vwap):
    ...
    x1 = rank(correlation(ts_sum(
        ((Open * 0.178404) + (low * (1 - 0.178404))), 13), ts_sum(adv120, 13), 17))
    x2 = rank(delta(((((high + low) / 2) * 0.178404) +
...
def alpha71(volume, close, low, Open, vwap):
    ...
    x1 = ts_rank(decay_linear(correlation(
        ts_rank(close, 3), ts_rank(adv180, 12), 18), 4), 16)
    x2 = ts_rank(decay_linear(
        (rank(((low + Open) - (vwap + vwap))).pow(2)), 16), 4)
    alpha = x1
    alpha[x1 < x2] = x2
    return alpha.fillna(value=0)
...
def alpha72(volume, high, low, vwap):
    ...
    x2 = rank(decay_linear(correlation(
        ts_rank(vwap, 4), ts_rank(volume, 19), 7), 3))
...
def alpha78(volume, low, vwap):
    adv40 = sma(volume, 40)
    x1 = rank(correlation(
        ts_sum(((low * 0.352233) + (vwap * (1 - 0.352233))), 20), ts_sum(adv40, 20), 7))
...
def alpha83(high, low, close, volume, vwap):
    x = rank(delay(((high - low) / (ts_sum(close, 5) / 5)), 2)) * \
        rank(rank(volume))
...
def alpha88(volume, Open, low, high, close):
    ...
    x1 = rank(decay_linear(
        ((rank(Open) + rank(low)) - (rank(high) + rank(close))), 8))
    x2 = ts_rank(decay_linear(correlation(
        ts_rank(close, 8), ts_rank(adv60, 21), 8), 7), 3)
    alpha = x1
    alpha[x1 > x2] = x2
    return alpha.fillna(value=0)
...
def alpha96(volume, vwap, close):
    ...
    x2 = ts_rank(decay_linear(ts_argmax(correlation(
        ts_rank(close, 7), ts_rank(adv60, 4), 4), 13), 14), 13)
    alpha = x1
    alpha[x1 < x2] = x2
    return alpha.fillna(value=0)
...
def alpha100(volume, close, low, high, ind):
    adv20 = sma(volume, 20)
    r1 = IndNeutralize(
        rank(((((close - low) - (high - close)) / (high - low)) * volume)), ind)
    r2 = 1.5 * scale(IndNeutralize(r1, ind))
    r3 = scale(IndNeutralize(
        (correlation(close, rank(adv20), 5) - rank(ts_argmin(close, 30))), ind))
